...
# Tests for gemini
su - dartfx -c 'export NVM_DIR="$HOME/.nvm"; source "$NVM_DIR/nvm.sh"; export PNPM_HOME="$HOME/.local/share/pnpm"; export PATH="$PNPM_HOME:$PATH"; gemini --version'

# Tests for claude
su - dartfx -c 'export NVM_DIR="$HOME/.nvm"; source "$NVM_DIR/nvm.sh"; export PNPM_HOME="$HOME/.local/share/pnpm"; export PATH="$PNPM_HOME:$PATH"; claude --version'
...
Rscript -e 'pkgs <- c("DBI", "RPostgres", "arrow", "data.table", "duckdb", "ellmer", "jsonlite", "rollama", "sdcMicro", "sf", "terra", "tidyverse"); missing <- pkgs[!vapply(pkgs, requireNamespace, logical(1), quietly = TRUE)]; if(length(missing) > 0) { stop("Missing R packages: ", paste(missing, collapse=", ")) } else { cat("[ok] all R packages found\n") }'

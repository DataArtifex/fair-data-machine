#!/usr/bin/env bash
# Builds a base+R derived image; auto-builds base first if missing.
set -euo pipefail
...
OUTPUT_IMAGE_NAME="${TOOLCHAIN_IMAGE_NAME:-kulnor/fair-data-machine-r}"
...
echo "[build:r] Building derived image: $OUTPUT_IMAGE from $BASE_IMAGE"
build_composed_image "$ROOT_DIR" "$OUTPUT_IMAGE" "$BASE_IMAGE" r

run_post_build_smoke_if_enabled "$ROOT_DIR" "$OUTPUT_IMAGE" "$RUN_POST_BUILD_TESTS" "build:r"

echo "[build:r] Done"
echo "[build:r] TOOLCHAIN_IMAGE=$OUTPUT_IMAGE"
